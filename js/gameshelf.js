...
.tier-box{width:52px;height:52px;border-radius:15px;display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:800;color:white;background:rgba(214,108,255,.14);border:1px solid rgba(214,108,255,.28);margin:auto}
.tier-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:10px}
.game-card{cursor:pointer;transition:.22s}
...
overlay.querySelector(".gm-rating").textContent=
game.rating!=null?`${game.rating}/10`:"—";

overlay.querySelector(".gm-status").textContent=
game.status??"—";

overlay.querySelector(".gm-hours").textContent=
game.hours??"—";

overlay.querySelector(".gm-deaths").textContent=
game.deaths??"—";
