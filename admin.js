(() => {
  ...
  const coverName = path => String(path || "").split("/").pop();
  ...
  function clearDragState() { document.querySelectorAll(".admin-dragging,.admin-drop-target,.admin-over").forEach(e=>e.classList.remove("admin-dragging","admin-drop-target","admin-over")); draggedGame=null; draggedCard=null; }
  ...
  function availableCovers(currentCover) {
    const used = new Set(games.map(g => coverName(g.cover)).filter(Boolean));
    return covers.filter(c => c.name === currentCover || !used.has(c.name));
  }
  ...
  function openEditor(gameName=null) {
    ...
    if(currentCover && ![...coverInput.options].some(o=>o.value===currentCover)){const option=document.createElement("option");option.value=currentCover;option.textContent=`${currentCover} (текущая)`;coverInput.append(option);}coverInput.value=currentCover||"";
    const updatePreview=()=>{preview.src=coverInput.value?`../covers/${coverInput.value}`:"";preview.style.display=coverInput.value?"block":"none";};coverInput.onchange=updatePreview;updatePreview();
  ...
  async function init(){
    ...
    try{
      ...
      document.querySelectorAll("#gameshelfContainer .game-card").forEach(decorateCard); setupRows();
