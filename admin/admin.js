...
const TIER_NAMES = {NOW:"Играю сейчас",S:"S",A:"A",B:"B",C:"C",D:"D",F:"F",PLANNED:"В планах"};
...
function render(){
  ...
  TIERS.forEach(tier=>{
    const row=document.createElement("section");row.dataset.tier=tier;row.style.cssText="display:grid;grid-template-columns:88px 1fr;gap:10px;align-items:stretch;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:8px;min-height:92px;transition:.15s;";
    const label=document.createElement("div");label.textContent=TIER_NAMES[tier];label.style.cssText="display:flex;align-items:center;justify-content:center;border-radius:12px;background:rgba(214,108,255,.12);font-weight:800;";
    const zone=document.createElement("div");zone.dataset.tier=tier;zone.style.cssText="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:7px;align-items:start;min-height:74px;padding:2px;border-radius:12px;";
    ...
    games.forEach((g,i)=>{
      ...
      const card=document.createElement("article");card.draggable=true;card.style.cssText="position:relative;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);border-radius:12px;overflow:hidden;cursor:grab;min-width:0;";
      ...
      card.querySelector(".edit").onclick=()=>edit(i);card.querySelector(".delete").onclick=()=>{games.splice(i,1);render();};zone.append(card);
    });
...
function edit(i){
  ...
  const updatePreview=()=>{preview.src=coverInput.value?coverPath(coverInput.value):"";preview.style.display=coverInput.value?"block":"none";};coverInput.onchange=updatePreview;updatePreview();
